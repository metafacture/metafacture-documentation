...
Running this little Flux script with the [Metafacture Runner CLI](https://github.com/metafacture/metafacture-core/releases) (with `flux.sh -f /path/to/your.flux` on Unix/Linux/Mac or `flux.bat -f /path/to/your.flux` on Windows) or on the [Metafacture Playground](https://metafacture.org/playground/?flux=%22Hello+World%22%0A%7C+print%0A%3B), generates the string `Hello World` on the standard output. We just asked Metafacture to print the incoming string.
...
To handle metadata records and ETL (Extract - Transform - Load) we have to build UNIX-like pipelines. For each step in a pipeline we use Flux commands/modules. Each Flux command performs a specific modular step in an ETL process that usually can be subsumed under one of the following steps:
...
Metafacture provides decoders and encoders for many formats. We also used the `as-records` module instead of `as-lines` to read multi-line metadata statements and not each line separately.

## OPTIONS

Each Metafacture module can have options that change its behavior. The options can be found in the documentation or by running the runner with the `-l`/`--list-commands` option.

As an example, we can use the JSON encoder option `prettyPrinting` to generate a pretty-printed version of the JSON:
...
In the example we created the Fix script `example.fix` that contains a combination of mappings and data conversion on (nested) data. We run a YAML to YAML conversion using the `example.fix` Fix script. (Note: unlike in Catmandu, array/list indexes are 1-based.)


## SPECIALIZATIONS

Metafacture was mainly created for data conversions of specialized metadata formats in the field of libraries, archives and museums. One of the specialized modules is `decode-marc` and `encode-marc`. With these you can read, write and convert MARC files.
